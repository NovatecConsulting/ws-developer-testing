...
internal class Isbn13Test : ValueTypeContract<Isbn13, String>() {
    ...
    inner class `during construction` {
        ...
        @ParameterizedTest
        fun `13 digit numbers are considered valid`(isbnCandidate: String) {
        ...
        @ParameterizedTest
        fun `any other number of digits is considered invalid`(isbnCandidate: String) {
            assertThrows<Isbn13.NotAnIsbnNumberException> {
        ...
        @ParameterizedTest
        fun `any non number characters are considered invalid`(isbnCandidate: String) {
            assertThrows<Isbn13.NotAnIsbnNumberException> {
                Isbn13(isbnCandidate)
            }
        }
    ...
    @Nested
    inner class `when parsing from a String` {

        @Test
        fun `10 digit ISBNs are converted to 13 digit ISBNs`() {
            val isbn = Isbn13.parse("0000000000")
            assertThat(isbn.toString()).isEqualTo("9780000000000")
        }

        @Test
        fun `13 digit ISBNs are used as they are`() {
            val isbn = Isbn13.parse("0000000000111")
            assertThat(isbn.toString()).isEqualTo("0000000000111")
        }

        @Test
        fun `13 digit ISBNs with separator are parsed`() {
