...
internal class BookIdTest : ValueTypeContract<BookId, String>() {
    ...
    @Test
    fun `toString() returns BookId's value as a String`() {
        val bookId = BookId(uuid)
        assertThat(bookId.toString()).isEqualTo("4b13ce6b-4546-4b89-9356-dd7bbf67c40d")
    }

    @Test
    fun `toUuid() returns BookId's value as a UUID`() {
        val bookId = BookId(uuid)
        assertThat(bookId.toUuid()).isEqualTo(uuid)
    }

    @Test
    fun `when generating IDs each generated ID is unique`() {
    ...
    @Nested
    inner class `when creating IDs from String` {

        @Test
        fun `UUID conform Strings are accepted`() {
            BookId.from("4b13ce6b-4546-4b89-9356-dd7bbf67c40d")
        }

        @Test
        fun `non UUID conform Strings will throw an exception`() {
            assertThrows<BookId.NotABookIdException> {
