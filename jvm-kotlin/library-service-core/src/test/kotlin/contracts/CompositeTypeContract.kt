...
abstract class CompositeTypeContract<out T : Any> {
     ...
     */
    @Nested
    inner class `is a composite type` {

        @Nested
        inner class `equality` {

            @Test
            fun `instances with different values are not considered equal`() {
            ...
            @Test
            fun `instances with equal values are considered equal`() {
        ...
        @Nested
        inner class `hash codes` {

            @Test
            fun `instances with different values have different hash codes`() {
                val instance = createExampleInstance()
                createOtherExampleInstances().forEach {
            ...
            @Test
            fun `instances with equal values share the same hash code`() {
                val instance1 = createExampleInstance()
                val instance2 = createExampleInstance()
